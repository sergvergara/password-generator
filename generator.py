...
def gen_usage(maxNumber):
    #extrachars=~!@#$%^&*()_+=-`<>,./?;:\'"[]{}|\\ 
    p = SimpleEngine(string_length=maxNumber, char_list='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890')
    counter = 0
    for password in p.generator():
        #print password
	passwordstring = "".join(str(x) for x in password)
        print passwordstring
        counter = counter + 1
        print "PASSWORD NUMBER - "+str(maxNumber)+' '+str(counter)
...
if __name__ == '__main__':
    ## Basic generator
    print 'generator'
    maxNumber = int(raw_input("Enter the maximum of characters: "))
    minNumber = int(raw_input("Enter the minimum of characters: "))
    gen_usage(maxNumber)
